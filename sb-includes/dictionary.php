...
function sb_search_results_dictionary() {
	return array(
		...
		'[date]' => '<?php echo date(get_option("date_format"), strtotime($sermon->date." 00:00 ".date("O"))) ?>',
		...
		'[podcasticon]' => '<img alt="Subscribe to full podcast" title="Subscribe to full podcast" class="podcasticon" src="<?php echo sb_get_value("plugin_url") ?>/sb-includes/icons/podcast.png"/>',
		'[podcasticon_for_search]' => '<img alt="Subscribe to custom podcast" title="Subscribe to custom podcast" class="podcasticon" src="<?php echo sb_get_value("plugin_url") ?>/sb-includes/icons/podcast_custom.png"/>',
...
function sb_sermon_page_dictionary() {
	return array(
		...
		'[date]' => '<?php echo date(get_option("date_format"), strtotime($sermon["Sermon"]->date." 00:00 ".date("O"))) ?>',
		...
		'[lbrvtext]' => '<?php for ($i = 0; $i < count($sermon["Sermon"]->start); $i++): echo sb_add_bible_text ($sermon["Sermon"]->start[$i], $sermon["Sermon"]->end[$i], "lbrv"); endfor ?>',
