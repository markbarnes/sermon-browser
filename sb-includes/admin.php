...
function sb_options() {
	...
	elseif (isset($_POST['save'])) {
		...
		sb_update_option('podcast_url', $_POST['podcast']);
		if (intval($_POST['perpage']) > 0)
			sb_update_option('sermons_per_page', $_POST['perpage']);
		...
		sb_update_option('upload_url', trailingslashit(get_bloginfo('wpurl')).$dir);
		...
		sb_update_option ('import_filename', $_POST['import_filename']);
	...
	<div class="wrap">
		...
		<table border="0" class="widefat">
			...
			<tr>
				...
				<td><input type="text" name="dir" value="<?php echo sb_get_option('upload_dir') ?>" style="width:100%" /></td>
			</tr>
			<?php
				} else {
			?>
				<input type="hidden" name="dir" value="<?php echo sb_get_option('upload_dir') ?>">
			<?php
				}
			?>
			<tr>
				<td align="right" style="vertical-align:middle"><?php _e('Public podcast feed', $sermon_domain) ?>: </td>
				<td><input type="text" name="podcast" value="<?php echo sb_get_option('podcast_url') ?>" style="width:100%" /></td>
			</tr>
			<tr>
...
function sb_manage_preachers() {
	...
	if (isset($_POST['save'])) {
		...
		if(!$error)
			echo "<script>document.location = '".get_bloginfo('wpurl')."/wp-admin/admin.php?page=sermon-browser/preachers.php&saved=true';</script>";
	}
			...
			<table class="widefat">
				...
				<tr>
					<td>
						...
						<div><img src="<?php echo trailingslashit(get_bloginfo('wpurl')).sb_get_option('upload_dir').'images/'.$preacher->image ?>"></div>
			...
			<tbody>
				<?php foreach ((array) $preachers as $preacher): ?>
					<tr class="<?php $i=0; echo (++$i % 2 == 0) ? 'alternate' : '' ?>">
						...
						<td style="text-align:center"><?php echo ($preacher->image == '') ? '' : '<img src="'.trailingslashit(get_bloginfo('wpurl')).sb_get_option('upload_dir').'images/'.$preacher->image.'">' ?></td>
						<td style="text-align:center">
							<a href="<?php echo get_bloginfo('wpurl') ?>/wp-admin/admin.php?page=sermon-browser/preachers.php&act=edit&pid=<?php echo $preacher->id ?>"><?php _e('Edit', $sermon_domain) ?></a> | <a onclick="return confirm('Are you sure?')" href="<?php echo get_bloginfo('wpurl') ?>/wp-admin/admin.php?page=sermon-browser/preachers.php&act=kill&pid=<?php echo $preacher->id ?>"><?php _e('Delete', $sermon_domain) ?></a>
						</td>
					</tr>
				<?php endforeach ?>
			</tbody>
...
?>
	<script type="text/javascript">
		...
		function createNewServices(s) {
			...
			if (s != null) {
				jQuery.post('<?php echo $_SERVER['PHP_SELF'] ?>?page=sermon-browser/sermon.php', {sname: s, sermon: 1}, function(r) {				
					if (r) {
						sz = s.match(/(.*?)@(.*)/)[1];
						t = s.match(/(.*?)@(.*)/)[2];
		...
		function createNewSeries(s) {
			...
			if (ss != null) {
				jQuery.post('<?php $_SERVER['PHP_SELF'] ?>?page=sermon-browser/sermon.php', {ssname: ss, sermon: 1}, function(r) {
					if (r) {
		...
		function deleteSeries(id) {
			jQuery.post('<?php echo $_SERVER['PHP_SELF'] ?>?page=sermon-browser/sermon.php', {ssname: 'dummy', ssid: id, del: 1, sermon: 1}, function(r) {
				if (r) {
		...
		function deleteServices(id) {
			jQuery.post('<?php echo $_SERVER['PHP_SELF'] ?>?page=sermon-browser/sermon.php', {sname: 'dummy', sid: id, del: 1, sermon: 1}, function(r) {
				if (r) {
		...
		function renameSeries(id, old) {
			...
			if (ss != null) {
				jQuery.post('<?php echo $_SERVER['PHP_SELF'] ?>?page=sermon-browser/sermon.php', {ssid: id, ssname: ss, sermon: 1}, function(r) {
					if (r) {
		...
		function renameServices(id, old) {
			...
			if (s != null) {
				jQuery.post('<?php echo $_SERVER['PHP_SELF'] ?>?page=sermon-browser/sermon.php', {sid: id, sname: s, sermon: 1}, function(r) {
					if (r) {
						sz = s.match(/(.*?)@(.*)/)[1];
						t = s.match(/(.*?)@(.*)/)[2];						
			...
			if ($headers[0] == 'HTTP/1.1 200 OK') {
				if ($_POST['import_type'] == 'download') {
					...
					if (file_exists(SB_ABSPATH.sb_get_option('upload_dir').$filename))
						...
						echo "<script>document.location = '".$_SERVER['PHP_SELF']."?page=sermon-browser/new_sermon.php&getid3=".$wpdb->insert_id."';</script>";
					}
				} else {
					$wpdb->query("INSERT INTO {$wpdb->prefix}sb_stuff VALUES (null, 'url', '{$url}', 0, 0, 0)");
					echo "<script>document.location = '".$_SERVER['PHP_SELF']."?page=sermon-browser/new_sermon.php&getid3=".$wpdb->insert_id."';</script>";
                    die();
				}
			} else
...
?>
	<script>
		function rename(id, old) {
			...
			if (f != null) {
				jQuery.post('<?php echo $_SERVER['PHP_SELF'] ?>?page=sermon-browser/uploads.php', {fid: id, oname: old, fname: f, sermon: 1}, function(r) {
					if (r) {
						if (r == 'renamed') {
		...
		function kill(id, f) {
			jQuery.post('<?php echo $_SERVER['PHP_SELF'] ?>?page=sermon-browser/files.php', {fname: f, fid: id, del: 1, sermon: 1}, function(r) {
				if (r) {
					if (r == 'deleted') {
		...
		function fetchU(st) {
			jQuery.post('<?php echo $_SERVER['PHP_SELF'] ?>?page=sermon-browser/uploads.php', {fetchU: st + 1, sermon: 1}, function(r) {
				if (r) {
		...
		function fetchL(st) {
			jQuery.post('<?php echo $_SERVER['PHP_SELF'] ?>?page=sermon-browser/files.php', {fetchL: st + 1, sermon: 1}, function(r) {
				if (r) {
		...
		function findNow() {
			jQuery.post('<?php echo $_SERVER['PHP_SELF'] ?>?page=sermon-browser/files.php', {search: jQuery('#search').val(), sermon: 1}, function(r) {
				if (r) {
				...
				<?php if (is_array($unlinked)): ?>
					<?php foreach ($unlinked as $file): ?>								
						<tr class="file <?php $i=0; echo (++$i % 2 == 0) ? 'alternate' : '' ?>" id="file<?php echo $file->id ?>">
							...
							<td style="text-align:center">
								<a id="" href="<?php echo $_SERVER['PHP_SELF']."?page=sermon-browser/new_sermon.php&amp;getid3={$file->id}"; ?>"><?php _e('Create sermon', $sermon_domain) ?></a> | 
...
function sb_ping_gallery() {
	...
	if((ini_get('allow_url_fopen') | function_exists('curl_init')) & get_option('blog_public') == 1 & get_option('ping_sites') != "") {
		...
		$url .= "&most_recent=".URLencode($wpdb->get_var("SELECT datetime FROM {$wpdb->prefix}sb_sermons ORDER BY datetime DESC LIMIT 1"));
...
?>	
	<script>
		function fetch(st) {
			jQuery.post('<?php echo $_SERVER['PHP_SELF'] ?>?page=sermon-browser/sermon.php', {fetch: st + 1, sermon: 1, title: jQuery('#search').val(), preacher: jQuery('#preacher').val(), series: jQuery('#series').val() }, function(r) {
				if (r) {
			...
			elseif ($_FILES['upload']['error'][$uid] == UPLOAD_ERR_OK) {
				...
				if ($file_allowed) {
					...
					if ($wpdb->get_var("SELECT COUNT(*) FROM {$wpdb->prefix}sb_stuff WHERE name = '$filename'") == 0 && move_uploaded_file($_FILES['upload']['tmp_name'][$uid], $dest)) {
						$filename = $prefix.mysql_real_escape_string($filename);
						$wpdb->query("INSERT INTO {$wpdb->prefix}sb_stuff VALUES (null, 'file', '$filename', $id, 0, 0)");
					} else {
...
?>
	<script type="text/javascript">		
		...
		function createNewPreacher(s) {
			...
			if (p != null) {
				jQuery.post('<?php echo $_SERVER['PHP_SELF']?>?page=sermon-browser/sermon.php', {pname: p, sermon: 1}, function(r) {
					if (r) {
		...
		function createNewSeries(s) {
			...
			var ss = prompt("New series' name?", "Series' name");
			if (ss != null) {
				jQuery.post('<?php echo $_SERVER['PHP_SELF']?>?page=sermon-browser/sermon.php', {ssname: ss, sermon: 1}, function(r) {
					if (r) {
...
?>	
	<div class="wrap">
        ...
        <h2>Thank you</h2>
        <p>A number of individuals and churches have kindly <a href="http://www.4-14.org.uk/wordpress-plugins/sermon-browser#support">donated</a> to the development of Sermon Browser. Their support is very much appreciated.</p>
	...
	if (sb_display_url() == "#") {
		...
		if (!function_exists('ap_insert_player_widgets')) {
			if ($wpdb->get_var("SELECT COUNT(id) FROM {$wpdb->prefix}sb_stuff WHERE name LIKE '%.mp3'")>0)
				echo '<div id="message" class="updated"><p><b>'.sprintf(__('Tip: Installing the %1$sWordpress Audio Player%2$s will allow users to listen to your sermons more easily.', $sermon_domain), '<a href="'.get_option('siteurl').'/wp-admin/plugin-install.php?tab=search&s=audio%20player&type=term&search=Search">', '</a>').'</b></div>';
		}
	}
}
...
function sb_rightnow () {
	...
	if ($file_count > 0) {
		...
		$output_string .= '<a href="'.get_bloginfo('wpurl').'/wp-admin/admin.php?page=sermon-browser/files.php">';
		$output_string .= sprintf(_n('%d file', '%d files', $file_count), $file_count)."</a> ";
		if ($sermon_count > 0) {
			$output_string .= __("in")." ".'<a href="'.$_SERVER['PHP_SELF'].'?page=sermon-browser/sermon.php">';
			$output_string .= sprintf(_n('%d sermon', '%d sermons', $sermon_count), $sermon_count)."</a> ";
		}
		if ($preacher_count > 0) {
			$output_string .= __("from")." ".'<a href="'.get_bloginfo('wpurl').'/wp-admin/admin.php?page=sermon-browser/preachers.php">';
			$output_string .= sprintf(_n('%d preacher', '%d preachers', $preacher_count), $preacher_count)."</a> ";
		}
		if ($series_count > 0) {
			$output_string .= __("in")." ".'<a href="'.get_bloginfo('wpurl').'/wp-admin/admin.php?page=sermon-browser/manage.php">';
			$output_string .= sprintf(__('%d series'), $series_count)."</a> ";
		}
		if ($tag_count > 0) 
			$output_string .= __("using")." ".sprintf(_n('%d tag', '%d tags', $tag_count), $tag_count)." ";
		if (substr($output_string, -1) == " ")
			$output_string = substr($output_string, 0, -1);
		if ($download_count > 0) 
			$output_string .= ". ".sprintf(_n('Only one file has been downloaded', 'They have been downloaded a total of %d times', $download_count), $download_count);
		if ($download_count > 1) {
			...
			$most_popular_title = '<a href="'.get_bloginfo('wpurl').'/wp-admin/admin.php?page=sermon-browser/new_sermon.php&mid='.$most_popular->sermon_id.'">'.stripslashes($most_popular->title).'</a>';
			$output_string .= ". ".sprintf(__('The most popular sermon is %s, which has been downloaded %s times'), $most_popular_title, $most_popular->c);
		}
		$output_string .= '.</p>';
	}
	echo $output_string;
...
function sb_print_import_options_message($long = FALSE) {
	...
	if (!sb_import_options_set()) {
		...
		printf (__ ('You will need to set the %s before you can import MP3s and pre-fill the Add Sermons form.', $sermon_domain), "<a href=\"{$_SERVER['PHP_SELF']}?page=sermon-browser/options.php\">".__('import options', $sermon_domain).'</a>');
	}
}
...
?>
	<table width="100%" cellspacing="2" cellpadding="5" class="widefat">
		<form method="post" enctype="multipart/form-data" action ="<?php $_SERVER['PHP_SELF']?>?page=sermon-browser/files.php" >
		<thead>
		<tr>
			<th scope="col" colspan="3"><?php if (sb_import_options_set()) printf(__("Select an MP3 file here to have the %s form pre-filled using ID3 tags.", $sermon_domain), "<a href=\"{$_SERVER['PHP_SELF']}?page=sermon-browser/new_sermon.php\">".__('Add Sermons', $sermon_domain).'</a>'); else _e('Upload file', $sermon_domain);?></th>
		</tr>
		</thead>
		<tbody>
		<tr>
