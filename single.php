<div class="sermon-browser-results">
	<h2><?php echo stripslashes($sermon["Sermon"]->title) ?> <span class="scripture">(<?php for ($i = 0; $i < count($sermon["Sermon"]->start); $i++): ?><?php echo sb_get_books($sermon["Sermon"]->start[$i], $sermon["Sermon"]->end[$i]) ?> <?php endfor ?>)</span></h2>
	...
	<?php foreach ((array) $sermon["Files"] as $file): ?>	
		<?php sb_print_file($file) ?>  
	<?php endforeach ?>
	<?php foreach ((array) $sermon["URLs"] as $url): ?>
		<?php sb_print_url($url) ?>  
	<?php endforeach ?>
